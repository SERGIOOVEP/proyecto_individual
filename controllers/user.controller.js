...
const updateUser = async (req, res) => {
    try {
        ...
        const { name_user, email, year_user } = req.body

        if (name_user == undefined || email == undefined || year_user == undefined) {
            res.status(400).json({ message: "Bad request. Please fill all fields" })
        }
        const updateUserData = { name_user, email, year_user }
...
const updatePass = async (req, res) => {
    try {
        ...
        const updateUserPass = { pass:passwordEncrypted }
